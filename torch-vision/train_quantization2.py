...
def main(args):
    ...
    # model_not_fused=Darknet('/home/maziar/WA/torch_to_onnx/yolo_cfg/yolov3.cfg').to('cuda:0')
    ...
    evaluate(model, criterion, data_loader_test, device=device)
    print("\n")
    print('Disabling observer for subseq epochs')
    ...
    model.eval()
    quantized_eval_model.eval()
    quantized_eval_model.cpu()
    

    # torch.onnx.export(d_model,dummy_input,'d_model_resnet50.onnx', verbose=False,keep_initializers_as_inputs=True, opset_version=10)
    torch.onnx.export(quantized_eval_model,dummy_input,'quantized_eval_model_resnet50.onnx', verbose=False,keep_initializers_as_inputs=True, opset_version=11,export_params=False,export_raw_ir=True)
