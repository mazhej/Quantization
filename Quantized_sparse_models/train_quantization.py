...
from DgPruning import KeepSparsity
...
def main(args):
    ...
    # ds = torch.utils.data.Subset(dataset, indices=list(range(5000)))
    ds = dataset
    ...
    # ds_test = torch.utils.data.Subset(dataset_test, indices=list(range(1000)))
    ds_test = dataset_test
    # test_sampler = torch.utils.data.SequentialSampler(ds_test)
    data_loader_test = torch.utils.data.DataLoader(
        ds_test, batch_size=args.eval_batch_size,
        sampler=test_sampler, num_workers=args.workers, pin_memory=True)

    print("Creating model", args.model)
    # when training quantized models, we always start from a pre-trained fp32 reference model
    # model = torchvision.models.quantization.__dict__[args.model](pretrained=True, quantize=args.test_only)
    model_not_fused = torchvision.models.quantization.__dict__[args.model](pretrained=True)
    ...
    print("Evaluating original model on eval dataset")
    ...
    # Add Sparsity Mask: This should be after prepare_qat
    KeepSparsity(model, 'weight')
    # 
    ...
    print("Evaluating model after prepare_qat")
    evaluate(model, criterion, data_loader_test, device=device)

    model.apply(torch.quantization.enable_observer)
    model.apply(torch.quantization.enable_fake_quant)

    print("Evaluating original model after applying enable_fake_quant enable_observer")
    evaluate(model, criterion, data_loader_test, device=device)
