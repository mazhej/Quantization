...
def test(cfg,
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # inf_out_dg = dg_model.forward(imgs.cpu().numpy())
    ...
    if save_json and map and len(jdict):
        ...
        cocoGt = COCO(glob.glob('/home/maziar/WA/MLdata/github/yolov3/data/coco/annotations/instances_val2014.json')[0])  # initialize COCO ground truth api
        cocoDt = cocoGt.loadRes('results.json')  # initialize COCO pred api

        imgIds_detection = [ann['image_id'] for ann in jdict]
        imgIds_unq = np.unique(imgIds_detection)
