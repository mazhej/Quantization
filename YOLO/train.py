import argparse
# from FuseHook import FuseHook, modified_fuse_known_modules
from FuseHook import *
...
act_quant = FakeQuantize.with_args(observer=MovingAverageMinMaxObserver, quant_min=0, quant_max=255,
                                            dtype=torch.quint8, qscheme=torch.per_tensor_affine, reduce_range=False)
weight_quant = FakeQuantize.with_args(observer=MinMaxObserver, quant_min=-128, quant_max=127,
                                                   dtype=torch.qint8, qscheme=torch.per_tensor_symmetric, reduce_range=False)

qconfig = QConfig(activation=act_quant, weight=weight_quant)
...
def train():
    ...
    nw= 8
    # train_sampler = torch.utils.data.RandomSampler(dataset,replacement=True, num_samples=1000)
    # dataset_sub = torch.utils.data.Subset(dataset, indices=list(range(5000)))
    ...
    # Testloader
    dataset_test = LoadImagesAndLabels(test_path, img_size_test, 1,
                                                                 hyp=hyp,
                                                                 rect=True,
                                                                 cache_images=opt.cache_images,
                                                                 single_cls=opt.single_cls)

    # test_sampler = torch.utils.data.RandomSampler(dataset_test,replacement=True, num_samples=100)
    # ds_test = torch.utils.data.Subset(dataset_test, indices=list(range(500)))
    ...
    # Initialize model
    original_model = Darknet('/home/maziar/WA/torch_to_onnx/yolo_cfg/yolov3.cfg').cuda()
   
    # Optimizer
    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups
    for k, v in dict(original_model.named_parameters()).items():
        if '.bias' in k:
            pg2 += [v]  # biases
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        # load model
        try:
            chkpt['model'] = {k: v for k, v in chkpt['model'].items() if original_model.state_dict()[k].numel() == v.numel()}
            original_model.load_state_dict(chkpt['model'], strict=False)
        ...
        load_darknet_weights(original_model, weights)

    # Mixed precision training https://github.com/NVIDIA/apex
    if mixed_precision:
        original_model, optimizer = amp.initialize(original_model, optimizer, opt_level='O1', verbosity=0)
    ...
    if device.type != 'cpu' and torch.cuda.device_count() > 1 and torch.distributed.is_available():
        ...
        original_model = torch.nn.parallel.DistributedDataParallel(original_model, find_unused_parameters=True)
        original_model.yolo_layers = original_model.module.yolo_layers  # move yolo layer indices to top level
    print("\n")
    print('evaluating original model')
    original_model.eval()
    results, maps = test.test(cfg,
                                        data,
                                        batch_size=batch_size * 8*4,
    ...
    # model.eval()
    model = fuse_modules(original_model, h.modules_to_fuse, inplace=False, fuser_func=modified_fuse_known_modules )
    model.eval()
    # model = copy.deepcopy(d_model).cuda()
    ...
    #######################################################################################
    print("\n")
    print('evaluating applying observer & QAT')
    results, maps = test.test(cfg,
                                        data,
                                        batch_size=batch_size * 8*4,
                                        img_size=img_size_test,
